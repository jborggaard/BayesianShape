...
#      @printf("%g\n",nTest)
  ...
  vectors = zeros(nNodes,2)

  nUnknownCounter = 0;
  for i=interiorNodes
    global nUnknownCounter = nUnknownCounter+1;
    vectors[i,1] = uvp[nUnknownCounter];

    nUnknownCounter = nUnknownCounter + 1;
    vectors[i,2] = uvp[nUnknownCounter];
  end

  for i=1:nInnerNodes
    vectors[innerNodes[i],1] = dirichletU[i];
    vectors[innerNodes[i],2] = dirichletV[i];
  end

  pressure = zeros(Float64,nNodes,1);
  #for i=(nInnerNodes+1):(nInnerNodes+nOuterNodes)
  #  vectors[outerNodes[i-nInnerNodes],1] = 0;
  #  vectors[outerNodes[i-nInnerNodes],2] = 0;
  #end
